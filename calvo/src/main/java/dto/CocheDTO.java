    ...
    public void insertarCoche(Coche coche){
        try {
            ...
            String queryFormateada = String.format(query, SchemaDB.TAB_USER,SchemaDB.COL_ID,SchemaDB.COL_MARCA,
                    SchemaDB.COL_MODELO,SchemaDB.COL_COLOR,SchemaDB.COL_MATRICULA,SchemaDB.COL_PRECIO,coche.getId(),
                    coche.getMarca(),coche.getModelo(),coche.getColor(),coche.getMatricula(),coche.getPrecio());
    ...
    public void borrarCoche(int id){
        try {
            ...
            if (filas > 0){
                ...
                System.out.println("No se encontro ningun coche");
            }
        } catch (SQLException e) {
    ...
    public void listarCoches(){
        try {
            ...
            while (resultSet.next()){
                System.out.printf("ID: %d | Marca: %s | Modelo: %s | Colo: %s | Matricula: %s | Precio: %d\n",
                        resultSet.getInt(SchemaDB.COL_ID),
                        resultSet.getString(SchemaDB.COL_MARCA),
                        resultSet.getString(SchemaDB.COL_MODELO),
                        resultSet.getString(SchemaDB.COL_COLOR),
