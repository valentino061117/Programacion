...
public class Restaurante {
    ...
    public void agregarPedido(Consumiciones consumicion, String nombre){
        Clientes cliente = estaCliente(nombre);
        // if cliente !=null
        if (!estaConsumicion(consumicion.getNombre(),cliente)){
            // false, por lo tanto no está, por lo tanto, la agrego
            agregarConsumicion(cliente,consumiciones);
        } else {
            ...
            if (respuesta.equalsIgnoreCase("s")){
                ...
                for (int i = 0; i < numero; i++) {
                    agregarConsumicion(clientes,consumiciones);
                }
